...
func (h *MovieHandler) HandlerFindAll(c *gin.Context) {
	movies := make([]models.Movie, 0, len(h.dataBase))
	for _, movie := range h.dataBase {
...
func (h *MovieHandler) Update(c *gin.Context) {
	...
	originalMovie, ok := h.dataBase[id]
	if !ok {
